...
function hasInvalidBlockArguments(markup: ContentForMarkup): boolean {
  return (
    hasReservedBlockArgument(markup) ||
    hasInvalidBlockStaticArguments(markup) ||
    hasInvalidBlockClosestArguments(markup)
  );
}
...
function hasInvalidBlockClosestArguments(markup: ContentForMarkup): boolean {
  let contextArgumentCount = 0;

  for (const arg of markup.args) {
    if (!arg.name.startsWith(CONTENT_FOR_CLOSEST_PREFIX)) continue;
    if (arg.value.type === NodeTypes.String) return true;
