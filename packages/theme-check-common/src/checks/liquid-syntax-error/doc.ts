...
export function checkDocParserError(error: Error, context: Context, source: string): void {
  ...
  // A doc tag that fails to close because it wraps another doc tag is a
  // nesting error, not a generic unclosed-tag error. `hasNestedDocTag`
  // ignores an `enddoc` that carries markup (a Ruby-accepted closer), so a
  // genuinely-later-unclosed doc still falls through to the raw passthrough.
  if (
    error.message === UNCLOSED_DOC_PARSER_ERROR &&
    hasNestedDocTag(docTagBody(source, startIndex))
  ) {
    context.report({
      message: 'Nested doc tags are not allowed',
      startIndex,
  ...
  context.report({ message: error.message, startIndex, endIndex });
}

// The body of a raw tag is everything after its opening `{% ... %}`. For an
// unclosed tag that runs to end-of-source, that is the remainder of the file.
function docTagBody(source: string, startIndex: number): string {
