...
async function lint(path: string, source: string) {
  // The check harness parses only `.liquid` files as LiquidHtml; every other
  // extension (`.json`, `.css`, ...) is treated as a JSON source (see
  // to-source-code.ts). Route each file to the check that will actually visit
  // it, so the Liquid-vs-JSON selection matches the harness's own file typing.
  if (path.endsWith('.liquid')) {
    return runLiquidCheck(MaxFileSize, source, path);
  }

  return runJSONCheck(MaxFileSizeJSON, source, path);
}
