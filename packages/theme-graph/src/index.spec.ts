...
import { getWebComponentMap, WebComponentMap } from './getWebComponentMap';
import {
  bind,
  buildThemeGraph,
  Dependencies,
...
describe('Module: index', () => {
  ...
  describe('Unit: buildThemeGraph', { timeout: 10000 }, () => {
    ...
    describe('with a valid theme graph', () => {
      ...
      it('infers entry points from the templates folder', () => {
        expect(graph.entryPoints).toHaveLength(1);
        expect(graph.entryPoints[0].uri).toBe(p('templates/index.json'));
      });
